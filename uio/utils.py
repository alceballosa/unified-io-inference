...
def tokens_to_regions(
    predicted_tokens, image_size, token_per_label=4
    ...
    while True:
        ...
        if not np.all(predicted_tokens[cur : cur + token_per_label] > BIN_START):
            ...
            raise ValueError()
        locations.append(
            vocab_size - predicted_tokens[cur : cur + token_per_label] - 100
        )
        cur += token_per_label
        label_end = cur
        while (
            label_end < len(predicted_tokens)
            and 1 < predicted_tokens[label_end] <= BIN_START
        ):
            label_end += 1
    ...
    locations = locations.reshape((-1, 2))[:, ::-1].reshape(
        (-1, token_per_label)
    )  # [yx to xy]
...
def extract_segmentation_masks(img, segmention_mode="coarse_color") -> List[np.ndarray]:
    ...
    if segmention_mode == "any_pixel":
        ...
        first_obj = np.logical_and(
            np.logical_not(background), np.abs(mean_diff).sum(-1) < 100
        )

...
def temperature_sample(
    ...
    all_logprob = unflatten_beam_dim(expanded_all_logprob, batch_size, num_decodes)
    ...
    return (
        jnp.take_along_axis(decodes, idxs, axis=1),
        jnp.take_along_axis(log_prob, jnp.squeeze(idxs, axis=-1), axis=-1),
        jnp.take_along_axis(all_logprob, idxs, axis=1),
    )
...
def _temperature_sample_single_trial(
    ...
    all_log_prob0 = jnp.zeros((batch_size, max_decode_len), dtype=jnp.float32)
    ...
    def sampling_loop_body_fn(state: SamplingLoopState) -> SamplingLoopState:
        ...
        i, sequences, cache, cur_token, ended, rng, log_prob, all_log_prob = state
        # Split RNG for sampling.
        rng1, rng2 = random.split(rng)
        # Call fast-decoder model on current tokens to get next-position logits.
        logits, new_cache = tokens_to_logits(cur_token, cache, i[0], sequences)
        ...
        one_hot_indices = jax.nn.one_hot(
            i, all_log_prob.shape[-1], dtype=next_log_prob.dtype
        )
        all_log_prob = all_log_prob + one_hot_indices * jnp.expand_dims(
            next_log_prob, -1
        )
        ...
        ended |= has_additional_eos | jnp.expand_dims(i >= max_decode_len - 1, axis=1)
...
def cache_map(fn, cache, apply_to_index: bool = False):
    ...
    keyvals = jax.tree_util.tree_map(fn, keyvals)
...
def add_beam_dim(x: jnp.ndarray, beam_size: int, offset: int = 0) -> jnp.ndarray:
...
def beam_init(
    ...
    all_logprobs0 = jnp.tile(
        jnp.array([0.0] + [0.0] * (beam_size - 1)), [batch_size, max_decode_len]
    )
    all_logprobs0 = jnp.reshape(all_logprobs0, [batch_size, beam_size, max_decode_len])
...
def beam_search(
    ...
    beam_search_init_state = beam_init(
        batch_size, beam_size, max_decode_len, cache, cache_offset
    )
    ...
    def beam_search_loop_body_fn(state: BeamState) -> BeamState:
        ...
        flat_logits, new_flat_cache = tokens_to_logits(
            flat_ids, flat_cache, state.cur_index, state.live_seqs
        )
        ...
        topk_log_probs, topk_indices = top_k_two_stage(flat_log_probs, k=beams_to_keep)
        ...
        one_hot_indices = jax.nn.one_hot(
            state.cur_index, state.all_logprobs.shape[2], dtype=state.all_logprobs.dtype
        )

        # curr_log_probs = top_alive_log_probs - state.live_logprobs
        def identity_fn(x):
            return x
        ...
        curr_log_probs = jax.lax.cond(
            jax.lax.eq(state.cur_index, 0),
            lambda: identity_fn(top_alive_log_probs),
            lambda: update_fn(top_alive_log_probs, state.live_logprobs),
        )

        one_hot_indices = jnp.reshape(one_hot_indices, [1, 1, -1])
        curr_log_probs = jnp.expand_dims(curr_log_probs, -1)
        all_logprobs = state.all_logprobs + one_hot_indices * curr_log_probs
        # Determine the top k beam indices from the original set of all beams.
        # --> [batch, beams]
        top_alive_indices = gather_beams(
            topk_beam_indices, new_topk_indices, batch_size, 2 * beam_size, beam_size
        )
    ...
    finished_logprobs = final_state.all_logprobs

    # Drop the first dummy 0 token.
    return finished_seqs[:, :, 1:], finished_scores, finished_logprobs

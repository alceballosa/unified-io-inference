...
class ModelRunner:
    ...
    def __init__(
        ...
        self.model = UnifiedIOModel(
            module, text_decoder_length=32, image_decoder_length=1
        )
        ...
        if self.compiled:
            if self._compiled_batch_fn is None:
                self._compiled_batch_fn = jax.jit(
                    self.model.predict_batch_with_aux, static_argnums=list(range(3, 9))
                )
    ...
    def run(
        ...
        assert len(input_images) == len(
            input_texts
        ), "Different number of text/image inputs"
        ...
        if not answer_options:
            ...
            if isinstance(answer_options[0], str):
                ...
                preprocessed = [
                    utils.preprocess_target_image(x) for x in answer_options
                ]
                ...
                output_options = np.tile(
                    np.expand_dims(output_options, 0), [bs, 1, 1, 1, 1]
                )
                # [batch, n_options, n_patches]
                output_options_mask = np.tile(
                    np.expand_dims(output_options_mask, 0), [bs, 1, 1]
                )
                batch["output_options"] = output_options
            ...
            if self.log_inputs:
                logging.info(
                    f"Running model text_inputs={input_texts} and "
        ...
        outputs = dict(
            ...
            text=output_text,
            image_tokens=np.array(out["image_tokens"])
            if "image_tokens" in out
            else None,
    ...
    def depth(self, image, num_decodes=None, beam_search=None) -> Dict:
        ...
        rescaled_image = utils.undo_image_preprocessing(
            out["image"][0], image.shape[:2]
        )
        return {
            "image": out["image"][0],
    ...
    def image_inpainting(
        self, image, location, replace_with: str, num_decodes=None
    ) -> Dict:
    ...
    def region_classification(
        self, image, location, num_decodes=None, answer_options=None
    ) -> Dict:
    ...
    def image_classification(
        self, image, num_decodes=None, answer_options=None
    ) -> Dict:
    ...
    def segmentation_based_generation(
        self, binary_masks: List[np.ndarray], labels: List[str], num_decodes=None
    ) -> Dict:
        ...
        text = " , ".join(
            f"{a} : {b}" for a, b in zip(labels, GEN_SEGMENTATION_COLOR_NAMES)
        )
        text = text.lower()

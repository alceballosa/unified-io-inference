...
class DecodeFnCallable(typing_extensions.Protocol):
    ...
    def __call__(
        ...
        *,
        ...
        eos_id: int,
        num_decodes: int,
        decode_rng: Optional[jnp.ndarray],
        **kwargs
...
class UnifiedIOModel(nn.Module):
    ...
    def predict_with_answer_options(
        ...
        for i in range(n_groups):
            output_options = batch["output_options"][
                :, i * max_options : (i + 1) * max_options
            ]
            ...
            encoder_position_embedding = decoding.flat_batch_beam_expand(
                encoder_position_embedding, num_option
            )
            ...
            if text_answer_options:
                ...
                text_decoder_targets = jnp.pad(
                    text_decoder_targets, [[0, 0], [0, 1]]
                )  # Add room for EOS

                text_decoder_masks = text_decoder_inputs > 0
                text_decoder_inputs = jnp.pad(text_decoder_inputs, [[0, 0], [1, 0]])
                text_decoder_masks = jnp.pad(
                    text_decoder_masks, [[0, 0], [1, 0]], constant_values=True
                )

                eos_mask = jnp.logical_and(
                ...
                image_decoder_masks = batch["output_options_masks"][
                    :, i * max_options : (i + 1) * max_options
                ]
                image_decoder_masks = image_decoder_masks.reshape(-1, 256)
                output_options = output_options.reshape(
                    [decoded_size] + list(output_options.shape[2:])
                )
                ...
                image_decoder_inputs = jnp.concatenate(
                    [
                        jnp.zeros((image_decoder_targets.shape[0], 1), dtype=jnp.int32)
                        + image_start_token,
                        image_decoder_targets[:, :-1],
                    ],
                    axis=1,
                )
            ...
            if text_answer_options:
                ...
                soft_targets = common_utils.onehot(
                    image_decoder_targets + vocab_size, image_logits.shape[-1]
                )
    ...
    def _compute_logits_from_slice(
        ...
        cur_index: int,
        live_seqs: jnp.ndarray,
        params: PyTreeDef,
        encoded_inputs: jnp.ndarray,
        encoder_masks: jnp.ndarray,
        text_length: int,
        image_length: int,
        logit_masks: jnp.ndarray = None,
    ) -> Tuple[jnp.ndarray, Mapping[str, jnp.ndarray]]:
        ...
        decoder_masks = jax.lax.cond(
            cur_index < text_length,
    ...
    def predict_batch_with_aux(
        ...
        if image_length == 256:
            ...
            img = self.module.apply(
                {"params": params}, method=self.module.decode_code, code_b=image_decodes
            )

            if return_all_decodes:
                img = jnp.reshape(img, decodes.shape[:2] + img.shape[1:])
                image_decodes = jnp.reshape(
                    image_decodes, decodes.shape[:2] + image_decodes.shape[1:]
                )

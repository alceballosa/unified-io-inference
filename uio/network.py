...
class AttnBlock(nn.Module):
    ...
    def __call__(self, x, training=False):
        ...
        w_ = jnp.einsum(
            "bqc,bkc->bqk", jnp.reshape(q, (b, h * w, c)), jnp.reshape(k, (b, h * w, c))
        )
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(cfg.num_res_blocks):
                ...
                if curr_res in cfg.attn_resolutions:
                    hs = AttnBlock(block_in, name=f"down_{i_level}_attn_{i_block}")(hs)

            if i_level != num_resolutions - 1:
                hs = Downsample(block_in, name=f"down_{i_level}_downsample")(hs)
                curr_res = curr_res // 2
...
class EncoderLayer(nn.Module):
    ...
    def __call__(
        ...
        encoder_bias = self.relative_embedding(txt_position_ids, img_position_ids, True)
        # Attention block.
...
class DecoderLayer(nn.Module):
    ...
    def __call__(
        ...
        decoder_bias = self.relative_embedding(
            text_decoder_positions, image_decoder_positions, False
        )
...
class Encoder(nn.Module):
    ...
    def __call__(
        ...
        if image_encoder_inputs.ndim == 4:
            img_emb = layers.space_to_depth(
                image_encoder_inputs, spatial_block_size=cfg.image_patch_size
            )
        else:
            img_emb = image_encoder_inputs

        txt_pos_emb = self.positon_embedding(txt_position_ids)
        img_pos_emb = self.positon_embedding(
            img_position_ids + cfg.encoder_max_text_length
        )

        if (
            image_encoder_inputs.ndim == 4
            and img_emb.shape[1] != cfg.encoder_max_image_length
            and img_emb.shape[1] != 1
        ):
            ...
            img_pos_emb = jax.image.resize(
                img_pos_emb, [1, target_size, target_size, cfg.emb_dim], "bicubic"
            )
        ...
        txt_emb = layers.LayerNorm(dtype=cfg.dtype, name="txt_emb_pre_ln")(txt_emb)

        img_emb = layers.LayerNorm(dtype=cfg.dtype, name="img_emb_pre_ln")(img_emb)

        position_embedding = jnp.concatenate([txt_pos_emb, img_pos_emb], axis=1)

        position_embedding = layers.LayerNorm(dtype=cfg.dtype, name="pe_pre_ln")(
            position_embedding
        )
        ...
        return (
            nn.Dropout(rate=cfg.dropout_rate)(x, deterministic=deterministic),
            position_embedding,
        )
...
class Decoder(nn.Module):
    ...
    def __call__(
        ...
        pos_scaling = float(cfg.emb_dim / cfg.num_heads) ** -0.5
        self_abs_pos_bias = (
            jnp.einsum("bqhd,bkhd->bhqk", self_pos_q, self_pos_k) * pos_scaling
        )
...
class Transformer(nn.Module):
    ...
    def encode(
        ...
        if image_encoder_inputs.ndim == 3:
            ...
            image_length = int(
                np.prod(image_encoder_inputs.shape[1:3]) / (cfg.image_patch_size**2)
            )
        ...
        if text_encoder_pos_ids is None:
            text_encoder_pos_ids = jnp.arange(
                text_encoder_inputs.shape[1], dtype=jnp.int32
            )
            text_encoder_pos_ids = jnp.expand_dims(text_encoder_pos_ids, axis=0)
            text_encoder_pos_ids = jnp.tile(text_encoder_pos_ids, [bs, 1])

        encoder_masks = jnp.concatenate(
            [text_encoder_masks, image_encoder_masks], axis=1
        )
    ...
    def decode(
        ...
        if text_decoder_segment_ids is not None:
            decoder_segment_ids = jnp.concatenate(
                [text_decoder_segment_ids, jnp.ones(image_decoder_masks.shape)], axis=1
            )
        else:
            decoder_segment_ids = None

        decoder_masks = jnp.concatenate(
            [text_decoder_masks, image_decoder_masks], axis=1
        )
        ...
        decoder_inputs = jnp.concatenate(
            [text_decoder_inputs, image_decoder_inputs], axis=1
        )

        if text_decoder_positions is None:
            text_decoder_positions = jnp.arange(
                text_decoder_inputs.shape[1], dtype=jnp.int32
            )[None, ...]
            image_decoder_positions = jnp.arange(
                image_decoder_inputs.shape[1], dtype=jnp.int32
            )[None, ...]
        else:
            image_decoder_positions = jnp.arange(
                image_decoder_inputs.shape[1], dtype=jnp.int32
            )[None, ...]
            image_decoder_positions = jnp.tile(
                image_decoder_positions, [image_decoder_inputs.shape[0], 1]
            )
        ...
        logging.info(
            f"Decode called with EncodeLen={encoded[0].shape[1]}, DecodeInputLen={decoder_inputs.shape[1]}"
        )
        logits = self.decoder(
            encoded,
    ...
    def sample(
        ...
        cfg = self.config
        encoder_decoder_mask = layers.make_attention_mask(
            jnp.ones_like(decoder_inputs), encoder_masks, dtype=cfg.dtype
        )

        if decoder_masks is not None:
            decoder_attn_mask = layers.make_decoder_mask(
                decoder_target_tokens=decoder_masks, dtype=cfg.dtype
            )
        else:
            decoder_attn_mask = None
    ...
    def __call__(
        ...
        *,
        ...
        if image_decoder_targets.shape[1] > 1:
            image_decoder_tokens = self.discrete_vae.get_codebook_indices(
                image_decoder_targets, vae_decode
        ...
        image_decoder_inputs = jnp.concatenate(
            [
                jnp.zeros((image_decoder_tokens.shape[0], 1), dtype=jnp.int32)
                + cfg.vocab_size
                - 1,
                image_decoder_tokens[:, :-1],
            ],
            axis=1,
        )
